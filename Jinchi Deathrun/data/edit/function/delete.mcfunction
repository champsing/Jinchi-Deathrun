...
execute unless entity @e[tag = edit_amst] run return run tellraw @s ["",{text:">> ",color: gold},{text:"找不到可刪除的標記盔甲座",color: red},{text:", 請先指定"},{text:"至少一個",color:gold},{text:"標記盔甲座"}]
playsound entity.slime.death ambient @p ~ ~ ~
tellraw @s ["",{text: ">> ", color: gold},{text:"已刪除"},{selector: "@n[type = armor_stand, tag = edit_amst]", color: gold}]
