...
tellraw @s ["",{text: ">> ", color: gold},{text: "  ⫸ "},{text: "[ 大廳 ]", color: gray, click_event: {action: "run_command", command: "/tp @s @n[tag = jd.lobby]"}, hover_event: {action: "show_text", value: "點擊可傳送"}}, "：", {score: {name:  "lobby", objective: "x"}}, ", ", {score: {name: "lobby", objective: "y"}}, ", ", {score: {name: "lobby", objective: "z"}}]