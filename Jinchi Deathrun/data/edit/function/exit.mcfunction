...
execute as @e[tag = edit_title] run data modify entity @s text set value '{"text":"進入編輯模式","color":"#a335bc","bold":true}'
...
execute as @e[type = armor_stand, tag = A_point] run data merge entity @s {Glowing:0b,CustomNameVisible:0b,Marker:1b,Invisible:1b}
execute as @e[type = armor_stand, tag = B_point] run data merge entity @s {Glowing:0b,CustomNameVisible:0b,Marker:1b,Invisible:1b}
