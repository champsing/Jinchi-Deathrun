...
execute unless entity @n[type = armor_stand, tag = red_base, tag = edit_amst] run return run tellraw @s ["",{text:">> ",color: gold},{text:"無法傳送",color: red},{text:", 請先指定"},{text:"A點",color:gold}]
tp @s @n[type = armor_stand, tag = red_base, tag = edit_amst]
