...
execute if entity @n[tag = red_base, tag = invulnerable, type = interaction] run return run tellraw @s[team = red] [{translate: jd.empty},{translate:"jd.console",color:gold},{text: "我方陣地已是無敵狀態",color: red}]
execute if entity @n[tag = blue_base, tag = invulnerable, type = interaction] run return run tellraw @s[team = blue] [{translate: jd.empty},{translate:"jd.console",color:gold},{text: "我方陣地已是無敵狀態",color: red}]
...
tellraw @a [{translate: jd.empty},{translate:"jd.console",color:gold},{selector:"@s"},{text:"使用了"},{text:"無敵陣地道具",color:gold},{text:" 該隊陣地無法攻陣"},{text:"20秒",color:gold}]
