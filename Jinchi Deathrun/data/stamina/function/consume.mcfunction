...
execute unless score 準備階段 system matches 1 if entity @e[type=marker,scores={mode=3}] if predicate stamina:lose_stamina/5 run xp add @s[team = !spec, level = 0..2] 1
...
execute unless score 準備階段 system matches 1 if entity @e[type=marker,scores={mode=3}] if predicate stamina:lose_stamina/2 run xp add @s[team = !spec, level = 8..9] 1