...
execute unless score 倒數 gamecore matches 0..5 if score 準備階段 gamecore matches 1 as @a[team = !spec] store result score @s score run scoreboard players operation @s emerald *= 十倍 readonly
...
execute if score 已準備人數 gameinfo = 玩家人數 menu run function game:prepare/countdown
