...
#---放置陣地---
execute at @n[type = armor_stand, tag = red_base] align xz run summon block_display ~ ~2.5 ~ {block_state: {Name: "smooth_quartz"}, Glowing: true, Tags: ["red_base"],CustomName:[{text:"紅隊陣地",color: red}],CustomNameVisible: true}
execute at @n[type = armor_stand, tag = blue_base] align xz run summon block_display ~ ~2.5 ~ {block_state: {Name: "smooth_quartz"}, Glowing: true, Tags: ["blue_base"],CustomName:[{text:"藍隊陣地",color: blue}],CustomNameVisible: true}
