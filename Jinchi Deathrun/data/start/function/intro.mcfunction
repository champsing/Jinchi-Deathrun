...
tellraw @a[gamemode = creative] ["",{text:">> ",color:gold},"若想解除安裝資料包，請點擊 ",{text:"此處",color:gold, hover_event: {action: "show_text", value: "/function start:uninstall/mode"}, click_event: {action: "run_command", command: "/function start:uninstall/mode"}}]
execute if score 操作模式 system matches 1 run tellraw @a[gamemode = creative] ["",{text:">> ",color:gold},{text:"若您擁有權限，您可以點擊"},{text:"",color:gold},{text:" "},{text:"此處",underlined: true,bold: true, color: aqua,click_event: {action: "run_command",command: "/function edit:give"},hover_event: {action: "show_text",value: [{text: "/function edit:give"}]}},{text:" "},{text:"來取得編輯場地的道具"}]
