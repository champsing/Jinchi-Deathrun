...
execute unless entity @e[tag = jd.lobby] run tellraw @a [{translate: jd.empty},{translate:"jd.console",color:gold},"點擊 ",{text:"此處",click_event: {action: "run_command",command: "/function start:activate"},hover_event: {action: "show_text",value: [{text: "/function start:activate"}]},color: aqua, underlined: true}," 生成大廳"]
