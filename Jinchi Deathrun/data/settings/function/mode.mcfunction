...
return run tellraw @s [{translate: jd.empty},{translate:"jd.console",color:gold},{text:"目前尚未開放這項功能，敬請期待！",color: red}]
...
#---不是創造---
execute as @s[gamemode = !creative] run return run tellraw @s[gamemode = !creative] [{translate: jd.empty},{translate:"jd.console",color:gold},{text:"您沒有權限更動這項設定。請使用"},{text:"創造模式",bold:true,color:gold},{text:"更改設定。"}]
...
execute if score 遊戲模式 menu matches 99999 run tellraw @a [{translate: jd.empty},{translate:"jd.console",color:gold},{text:"對戰模式",bold:true,color:"#a335bc"},{text:"：雙方在時限內各自與敵方玩家對戰，以全隊分數決勝負。"}]
